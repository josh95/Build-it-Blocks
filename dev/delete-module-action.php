...
<body>
		...
		$temp = mysqli_query($con, "SELECT * FROM `module_index` WHERE `module_index`.`name` = \"$delete\"");
		...
		mysqli_query($con, "DELETE FROM `applications` WHERE `applications`.`moduleID` = $idnumber");
		mysqli_query($con, "DELETE FROM `steps` WHERE `steps`.`moduleID` = \"$idnumber\"");
		mysqli_query($con, "DELETE FROM `module_index` WHERE `module_index`.`name` = \"$delete\"");
